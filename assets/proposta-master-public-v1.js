(()=>{
  ...
  const statusEl=document.getElementById('acceptStatus'),form=document.getElementById('acceptForm'),btn=document.getElementById('acceptButton'),cpfEl=document.getElementById('acceptCpf');
  let proposal=null;
  ...
  if(cpfEl)cpfEl.addEventListener('input',()=>{let v=digits(cpfEl.value).slice(0,11);v=v.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');cpfEl.value=v});
  ...
  async function loadMaster(){
    ...
    const [htmlRes,dataRes]=await Promise.all([
      fetch('/master-template/proposta-master-limpa-v1.html?v=6',{cache:'no-store'}),
    ...
    await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='/master-template/proposta-master-limpa-v1.js?v=6';s.onload=resolve;s.onerror=()=>reject(new Error('Não foi possível carregar os dados da proposta.'));document.body.appendChild(s)});
  ...
  form.addEventListener('submit',async e=>{
    ...
    const nome=document.getElementById('acceptName').value.trim(),cpf=document.getElementById('acceptCpf').value.trim(),email=document.getElementById('acceptEmail').value.trim(),accepted=document.getElementById('acceptCheck').checked,cnpj=digits(proposal?.client?.cnpj||'');
    if(!nome){statusEl.className='accept-status error';statusEl.textContent='Informe o nome do sócio/responsável.';return}
    if(!validCpf(cpf)){statusEl.className='accept-status error';statusEl.textContent='Informe um CPF válido do sócio/responsável.';return}
    ...
    try{
      const r=await fetch('/api/proposal-acceptance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({cnpj,nome,cpf:digits(cpf),email,accepted:true,proposal_ref:ref})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Não foi possível registrar o aceite.');

...
assert.match(dadosV4,/brand-logo[^}]*width:min\(620px/i,'marca FÊNIX deve ser protagonista no topo');
assert.match(dadosV4,/hero h1[^}]*font-size:clamp\(22px,2\.4vw,30px/i,'Levantamento de dados deve ficar menor que a marca');
assert.match(dadosV4,/theme-toggle/,'coleta deve permitir alternar entre tema claro e escuro');
assert.match(dadosV4,/fenix-intake-theme/,'preferência de tema deve ser persistida');
...
console.log('PASS /dados oficial → marca protagonista + tema claro/escuro + endereço CNPJ + Intake 2.1 → Pipeline Dados recebidos');

...
function fitClientName(){const el=document.getElementById('clientName');if(!el)return;el.style.fontSize='4.7vw';let size=parseFloat(getComputedStyle(el).fontSize);const maxH=parseFloat(getComputedStyle(el).lineHeight)*3.03;while((el.scrollHeight>maxH+2||el.scrollWidth>el.clientWidth+2)&&size>46){size-=2;el.style.fontSize=size+'px';}}
...
function fitScope(opCount,mgrCount){const split=document.getElementById('scopeSplit');if(!split)return;split.classList.remove('dense','very-dense');const total=opCount+Math.max(1,mgrCount);if(total>=16)split.classList.add('very-dense');else if(total>=11)split.classList.add('dense');}
...
function fill(p){const c=p.client||{},t=p.commercial_terms||{},s=p.scope||{};
  ...
  const opCount=list('operationalScope',s.operational||[],'Nenhuma atividade operacional aprovada.');const mgrCount=list('managerialScope',s.managerial||[],'Nenhum item gerencial adicional aprovado neste pacote.');fitScope(opCount,mgrCount);
  ...
  text('commercialNote','Conta bancária adicional: '+money(t.additional_bank_account||0)+'/mês. Serviços extraordinários, retrabalhos ou atividades fora do escopo somente após autorização prévia do cliente.');
